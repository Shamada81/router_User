...
class User{
  static function addUser(){
    $mysqli = mysqli_connect("localhost", "levadny02_train", "olUuFf&8", "levadny02_train");
    $name = $_POST['name'];
  ...
  static function authUser(){
    $mysqli = new mysqli("localhost", "levadny02_train", "olUuFf&8", "levadny02_train");
    $email = mb_strtolower(trim($_POST['email']));
    $pass = trim($_POST['pass']);
    $result = $mysqli->query("SELECT * FROM `users` WHERE `email`='$email'");
  ...
  static function changeUserData(){
    $mysqli = new mysqli("localhost", "levadny02_train", "olUuFf&8", "levadny02_train");
    $value = $_POST['value'];
    $item = $_POST['item'];//здесь либо name либо lastname
    $userId = $_SESSION['id'];
